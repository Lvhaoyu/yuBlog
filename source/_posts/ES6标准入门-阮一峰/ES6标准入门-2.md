...
`ES6`允许按照一定模式从一些数据结构中提取值，然后对变量进行赋值，这就是解构赋值。
...
如果解构不成功，变量的值就会返回`undefined`。如果等号右边不是可遍历的机构，就会报错。

对于`Set`解构，也可以进行解构。所以能否解构完全取决于等号右边的数组是否有`Iterator`（遍历器）接口。`Generator`函数也可以。

![](111.jpg)

#### 完全解构

以下情况属于完全解构，就是变量和要结构的值刚好可以对应。
...
如果默认值是一个表达式，那么这个表达式是默认求值的，也就是只有用到的时候才会将这个表达式求值。
...
函数`f()`根本不会执行，只有解构失败才会取默认值。

默认值可以引用解构赋值的其他变量，但是必须是已经声明的变量。解构赋值也是一种声明变量的方式嘛。
...
也就是说，对象的解构赋值内部机制是先找到同名的属性，再赋值给对应的变量，真正被复制的是后者，而不是前者。

这也就解释了本节第一行代码为什么`a`和`b`会被显示为未声明。因为`a`和`b`是匹配的模式，`c`和`d`才是真正被赋值的的变量。
...
解构赋值允许等号左边的模式之中不放任何变量名称，这样写出的表达式没任何意义，但是不报错。
...
使用括号括起来的原因是让编译器认为这是一个表达式，而不是函数语句。
...
对以上两种类型的值进行解构赋值时，会对以上两者先转为对象，再进行解构赋值。
...
注意，这里面默认值是针对解构出来的`x`和`y`的。参数是`{}`，和参数的默认值无关。
...
这段代码就是给函数的参数指定默认值，不是给解构出来的变量默认值。具体流程是首先对函数的参数进行解构赋值，之后函数内部按照解构赋值出来的变量进行运算。

而函数参数默认值是针对于整个参数整体的，因为参数就是一个对象（就一个参数），所以只有当该对象不存在或者是`undefined`，才会触发默认值语句。
